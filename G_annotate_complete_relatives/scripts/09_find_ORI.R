...
# THIS DID NOT WORK I COULD NOT FIND AN ORI THAT WAY!
...
prot_df <- fread("improved_gene_df_with_DRAM_and_nr.tsv")


# search replication proteins ---------------------------------------------

search_terms <- paste(c("pMB1", "pBR322", "pBR322", "ColE1", "R6k", "p15A",
                        "pSC101", "pir", "pUC"),
                      collapse = "|")
ori_df <- data.table()

for(acc in unique(prot_df$contig)){
    tmp_df <- prot_df %>% 
        filter(contig == acc) %>% 
        filter(grepl(pattern = search_terms, x = interpro_annot, ignore.case = TRUE))
    ori_df <- rbind(ori_df, tmp_df)
