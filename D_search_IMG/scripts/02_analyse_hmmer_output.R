...
setwd("~/projects/pR1SE_australian_lakes/D_search_IMG/scripts")
...
big_df$host_taxonomy <- all_seq_info$`Host taxonomy prediction`[match(big_df$uvig, all_seq_info$UVIG)]
big_df$ecosystem <- all_seq_info$`Ecosystem classification`[match(big_df$uvig, all_seq_info$UVIG)]
big_df$taxonomy <- all_seq_info$`Taxonomic classification`[match(big_df$uvig, all_seq_info$UVIG)]

fwrite(big_df, "identified_viruses_with_info.tsv")
...
# we didnt do the following, since none 
# # generate presence absence plot ------------------------------------------
# 
# big_df$contig <- big_df$hit                         # lets see what we do here
...
# 
# 
# ggsave("../../plots/IMG_presence_absence_plot_novel.png", plot = last_plot(), height = 8, width = 16)
