...
ALL_OTHER_ORFS <- c("ORF8", "ORF10", "ORF17", "ORF21", "ORF23", "ORF24")
...
for(i in 1:nrow(region_df)){
    ...
    # loop over ORFs but not ORF6
    for(ORF in ALL_OTHER_ORFS){
        ...
        relevant_sequence_names <- prot_df %>% 
            ...
            filter(str_detect(annot, pattern = ORF)) %>% 
            select(id) %>% 
            pull()
        ...
        out_file <- paste0("proteins/", ORF, "_combined.faa")
